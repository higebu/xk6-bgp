...
func TestBuildUpdateMessage_AdvertiseRoundTrip(t *testing.T) {
	msg, err := BuildUpdateMessage(bgp.RF_IPv4_UC, false,
		PathAttrs{NextHop: parseAddr("10.0.0.1"), LocalAS: 65001},
		[]Route{
			{Prefix: mustPrefix("10.100.0.0/24")},
			{Prefix: mustPrefix("10.100.1.0/24")},
		}, EncodingOptions{})
	if err != nil {
...
func TestBuildUpdateMessage_WithdrawRoundTrip(t *testing.T) {
	msg, err := BuildUpdateMessage(bgp.RF_IPv4_UC, true, PathAttrs{},
		[]Route{{Prefix: mustPrefix("10.100.0.0/24")}}, EncodingOptions{})
	if err != nil {
		t.Fatalf("BuildUpdateMessage: %v", err)
	}
	buf, err := msg.Serialize()
...
func TestBuildUpdateMessage_IPv6AdvertiseRoundTrip(t *testing.T) {
	msg, err := BuildUpdateMessage(bgp.RF_IPv6_UC, false,
		PathAttrs{NextHop: parseAddr("2001:db8::1"), LocalAS: 65001},
		[]Route{
			{Prefix: mustPrefix("2001:db8:1::/48")},
			{Prefix: mustPrefix("2001:db8:2::/48")},
			{Prefix: mustPrefix("2001:0db8:3::/48")}, // non-canonical input
		}, EncodingOptions{})
	if err != nil {
		t.Fatalf("BuildUpdateMessage: %v", err)
	}
...
func TestBuildUpdateMessage_IPv6WithdrawRoundTrip(t *testing.T) {
	msg, err := BuildUpdateMessage(bgp.RF_IPv6_UC, true, PathAttrs{},
		[]Route{{Prefix: mustPrefix("2001:db8:1::/48")}}, EncodingOptions{})
	if err != nil {
		t.Fatalf("BuildUpdateMessage: %v", err)
	}
	buf, err := msg.Serialize()
...
func TestChunkRoutes_IPv6(t *testing.T) {
	...
	chunks, err := ChunkRoutes(bgp.RF_IPv6_UC, false, attrs, routes, EncodingOptions{})
	if err != nil {
	...
	for _, c := range chunks {
		msg, err := BuildUpdateMessage(bgp.RF_IPv6_UC, false, attrs, c, EncodingOptions{})
		if err != nil {
			t.Fatalf("BuildUpdateMessage: %v", err)
		}
		buf, err := msg.Serialize()
...
func TestChunkRoutes_ExtendedMessages(t *testing.T) {
	...
	chunks, err := ChunkRoutes(bgp.RF_IPv4_UC, false, attrs, routes, EncodingOptions{})
	if err != nil {
	...
	chunksExt, err := ChunkRoutes(bgp.RF_IPv4_UC, false, attrs, routes, EncodingOptions{UseExtendedMessages: true})
	if err != nil {
	...
	msg, err := BuildUpdateMessage(bgp.RF_IPv4_UC, false, attrs, chunksExt[0], EncodingOptions{UseExtendedMessages: true})
	if err != nil {
		t.Fatalf("BuildUpdateMessage: %v", err)
	}
...
func TestBuildUpdateMessage_FamilyMismatch(t *testing.T) {
	_, err := BuildUpdateMessage(bgp.RF_IPv4_UC, false,
		PathAttrs{NextHop: parseAddr("10.0.0.1"), LocalAS: 65001},
		[]Route{{Prefix: mustPrefix("2001:db8::/64")}}, EncodingOptions{})
	if err == nil {
...
func TestBuildUpdateMessage_IPv4UnicastForceMpReach(t *testing.T) {
	msg, err := BuildUpdateMessage(bgp.RF_IPv4_UC, false,
		PathAttrs{NextHop: parseAddr("10.0.0.1"), LocalAS: 65001},
		[]Route{{Prefix: mustPrefix("10.100.0.0/24")}},
...
func TestBuildUpdateMessage_IPv6AdvertiseNoIPv4NextHop(t *testing.T) {
	msg, err := BuildUpdateMessage(bgp.RF_IPv6_UC, false,
		PathAttrs{NextHop: parseAddr("2001:db8::1"), LocalAS: 65001},
		[]Route{{Prefix: mustPrefix("2001:db8:1::/64")}},
		EncodingOptions{})
	if err != nil {
		t.Fatalf("BuildUpdateMessage: %v", err)
	}
